...
5. [KVM 지원 확인](#5-kvm-지원-확인-중첩-가상화)
6. [Docker 설치](#6-docker-설치)
7. [롤백 & 삭제](#롤백--삭제)
...
> 💡 **왜 확인해야 하나요?**
>
> - OpenStack Nova는 VM을 생성할 때 가상화 기술을 사용합니다
> - **KVM**(하드웨어 가속): 빠른 성능, 프로덕션 권장
> - **QEMU**(소프트웨어 에뮬레이션): 느림 (KVM 대비 ~10배), 테스트용
>
> NHN Cloud m2 인스턴스는 대부분 중첩 가상화를 지원합니다.

```bash
# CPU가 가상화를 지원하는지 확인 (vmx=Intel, svm=AMD)
...
| 출력 값    | 의미          | 다음 단계                                                  |
| ---------- | ------------- | ---------------------------------------------------------- |
| **1 이상** | ✅ KVM 지원   | globals.yml에서 별도 설정 불필요 (자동 감지)               |
| **0**      | ❌ KVM 미지원 | globals.yml에서 `nova_compute_virt_type: "qemu"` 설정 필요 |

> 📝 **메모**: 위 결과를 기억해두세요! Step 2의 globals.yml 설정 시 사용합니다.
